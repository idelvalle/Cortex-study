...
rm(list=ls())
setwd("~/Desktop/presentation/deseq2_all/Cortex/")
...
coldata <- data.frame(row.names=colnames(countdata), stage, condition, batch)
coldata
...
write.csv(as.data.frame(stage.all.wpc), file="results.cortex.subset.ALL.csv")
...
write.table(loadings, file="loadings.csv", sep=",")
...
save.image(file="Cortex.subset.XY.vs.XX.RData")
